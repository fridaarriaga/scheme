...
def do_define_form(expressions, env):
    #structure of the expressions argument --> Pair(A, Pair(B, nil)), where: A is the symbol being bound, B is an expression whose value should be evaluated and bound to A
...
def do_quote_form(expressions, env): # What is the structure of the expressions argument to do_quote_form? ->Pair(A, nil), where: A is the quoted expression
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return True
    elif is_scheme_true(expressions.first) and expressions.rest is nil:
